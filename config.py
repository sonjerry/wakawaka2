...
ESC_MIN_US      = 1000 # 일반적인 ESC 최소값으로 조정
ESC_NEUTRAL_US  = 1500 # 일반적인 ESC 중립값으로 조정
ESC_MAX_US      = 2000 # [수정] 200 -> 2000. 일반적인 ESC의 최대 PWM 신호폭으로 수정
ESC_TRIM_US     = 0

# 논리 입력(-1..1)을 물리 ESC 펄스로 변환하기 위한 맵
ESC_DEADZONE_NORM   = 0.02   # |명령|<=2%면 진짜 중립 펄스 유지
ESC_FWD_START_NORM  = 0.05   # 전진 시작 임계 (필요 시 미세조정)
ESC_REV_START_NORM  = 0.05   # 후진 시작 임계 (필요 시 전/후진 따로 조정)

# ==== 3. 시뮬레이션 및 엔진/변속 설정 ====
TICK_S          = 0.01       # 시뮬레이션/제어 루프 주기
ARM_NEUTRAL_S   = 1.8        # ESC 아밍을 위한 중립 펄스 유지 시간
